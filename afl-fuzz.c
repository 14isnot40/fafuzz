...
int select_algorithm(int tmp_firefly)
{
  ...
  if (j == 1 && sele < queue_cur->prob_now[tmp_firefly][i - 1])
...
static void show_stats(void)
{
  ...
  if (fuzzing_mode == 1)
  {
    ...
    {
      sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "FA-AFL + local best firefly", use_banner);
    }
    else
    {
      sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "FA-AFL + firefly disturb", use_banner);
    }
  }
  else
  {
  ...
  fa_havoc_stage_entry:
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
    {
      ...
      for (i = 0; i < use_stacking; i++)
      {
        ...
        switch (select_algorithm(tmp_firefly))
        {
...
static u8 fa_updating(char **argv)
{
  ...
  if (last_update_round != ROUND_MAX)
    queue_cur->find_best_firefly = 1;
  else
